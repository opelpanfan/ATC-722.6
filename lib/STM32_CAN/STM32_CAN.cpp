...
int STM32_CAN::write(CAN_message_t &CAN_tx_msg)
{
  ...
  if (_channel == _CAN1)
  {
    ...
    if (mailbox >= 3) {
      ...
      return -1; // transmit of the previous messages has failed
    }
  ...
  if (_channel == _CAN2)
  {
    ...
    if (mailbox >= 3) {
      ...
      CAN2->sTxMailBox[mailbox].TDLR  = (((uint32_t) CAN_tx_msg.buf[3] << 24) |
                                         ((uint32_t) CAN_tx_msg.buf[2] << 16) |
                                         ((uint32_t) CAN_tx_msg.buf[1] <<  8) |
                                         ((uint32_t) CAN_tx_msg.buf[0]      ));
      CAN2->sTxMailBox[mailbox].TDHR  = (((uint32_t) CAN_tx_msg.buf[7] << 24) |
                                         ((uint32_t) CAN_tx_msg.buf[6] << 16) |
                                         ((uint32_t) CAN_tx_msg.buf[5] <<  8) |
                                         ((uint32_t) CAN_tx_msg.buf[4]      ));
      // Send Go
...
void STM32_CAN::setBaudRate(uint32_t baud)
{
  ...
  else if (_channel == _CAN2)
  {
    ...
    // Filter assigned to FIFO 0 
    // Filter bank register to all 0
    CANSetFilter(15, 1, 0, 0, 0x0UL, 0x0UL); 
    ...
    // Filter assigned to FIFO 0 
    // Filter bank register to all 0
    CANSetFilter(27, 1, 0, 0, 0x0UL, 0x0UL);
  }
  #endif
